...
## Introduction
When developing a video game, whether a single player or multiplayer one, social and competitive features have become  necessary to create a network effect and increase players' engagement. These features usually require a backend API to store players' data. To make the best usage of them, presence information will let players know about online status changes of other players, to be able to challenge them quickly, or invite them for a game session.

AWS provides developers with a wide spectrum of choice to develop such features, from plain Amazon EC2 instances based servers to containers and serverless. Among those, [AWS AppSync](https://aws.amazon.com/appsync) simplifies application development by letting you create a flexible API to securely access, manipulate, and combine data from one or more data sources. AppSync is a managed service that uses GraphQL to make it easy for applications to get exactly the data they need. One interesting feature of Amazon AppSync is real time updates, with which you can notify players about changes on the backend. 
...
Regarding the data itself, we want to store information about each player to know if they are still online or not. This can be translated into *the last hearbeat did not happen before the given timeout*. This information is similar to session information you would have on a website, and Amazon Elasticache is a good fit for this use case. (Ref: [Session Management](https://aws.amazon.com/caching/session-management/)). The key/value cache could store a player id as the key, and the heartbeat as the value. We also want to be able to quickly retrieve the session that have expired during a time interval, which leads to the choice to use [Redis Sorted Sets](https://redis.io/topics/data-types), using operations such as `ZADD`, `ZSCORE`, `ZRANGEBYSCORE`, or `ZREMRANGEBYSCORE`.
